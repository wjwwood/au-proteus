...
// (courtesy of Arduino forum user *etracer*)
  ...
  if (/*_inverse_logic ? rx_pin_read() :*/ !rx_pin_read())
  {
    ...
    //if ((_receive_buffer_tail + 1) % (_NewSS_MAX_RX_BUFF) != _receive_buffer_head) 
    {
      ...
      _receive_buffer_tail = (_receive_buffer_tail + 1) & (_NewSS_MAX_RX_BUFF-1);
			//_receive_buffer_tail %= (_NewSS_MAX_RX_BUFF);
    } 
		/*
    else 
    {
...
inline uint8_t NewSoftSerial::rx_pin_read()
{
...
int NewSoftSerial::read(void)
{
  ...
  _receive_buffer_head = (_receive_buffer_head + 1);
	_receive_buffer_head %= (_NewSS_MAX_RX_BUFF);
  return d;
}
