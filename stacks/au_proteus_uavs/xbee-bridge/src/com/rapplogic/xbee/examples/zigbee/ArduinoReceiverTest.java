 ...
 * 
 * To run, connect to your end device to your computer start.  You will start receiving packets when you start
 * ZNetSenderTest.java (connected to your coordinator).  You can start ZNetSenderTest.java and this class in 
 * any order but it's generally best to start this class first.
 * 
 * @author andrew
 *
 */
public class ArduinoReceiverTest {

	private final static Logger log = Logger.getLogger(ArduinoReceiverTest.class);
	
	private ArduinoReceiverTest() throws Exception {
		XBee xbee = new XBee();		
			...
			while (true) {
				...
				try {
					...
					if (response.getApiId() == ApiId.ZNET_RX_RESPONSE) {
						...
						// convert 10bit reading
						int pin5 = ByteUtils.parse10BitAnalog(rx.getData()[0], rx.getData()[1]);
						
						log.info("Received RX packet, option is " + rx.getOption() + ", sender 64 address is " + ByteUtils.toBase16(rx.getRemoteAddress64().getAddress()) + ", remote 16-bit address is " + ByteUtils.toBase16(rx.getRemoteAddress16().getAddress()) + ", data is " + ByteUtils.toBase16(rx.getData()) + ", pin5 10bit reading is " + pin5);
