...
//variables for handling serial input from x86 (ISR)
char cmdState=0, cmd_new_packet=0, cmd_buf_idx=0, cmd_read_idx=0, cmd_write_idx=0;
char inFromSerial[2][MAX_CMD_LEN]; //double buffer  
...
interrupt 20 void SCI0Handler(void){ char data;
  ...
  if(SCI0SR1 & RDRF){ 
    ...
    switch(cmdState){ //what do we do with new data packet?
      case INACTIVE:
        if(data == PROTEUS_BEGIN) {
          //LED_GREEN1 = 1;
          cmdState = ACTIVE; //begin new command
        } 
        break;
      case ACTIVE:
        if(data == PROTEUS_END) {
          if(cmd_new_packet) {
            LED_RED3 = 1; //error, not finished processing previous packet  
          }
          cmdState = INACTIVE; //end of command
          cmd_buf_idx = 0;
          cmd_new_packet = 1;
          ...
          inFromSerial[cmd_write_idx][cmd_buf_idx++] = data;
        }
        break;
      default: break;
    }
