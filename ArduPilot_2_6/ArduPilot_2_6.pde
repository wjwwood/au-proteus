...
//-----------------------
float kp[]={
  SERVO_ROLL_P,SERVO_PITCH_P,NAV_ROLL_P,NAV_PITCH_P,THROTTLE_P,THROTTLE_FBW_P}		
,ki[]={
  SERVO_ROLL_I,SERVO_PITCH_I,NAV_ROLL_I,NAV_PITCH_I,THROTTLE_I,THROTTLE_FBW_I}
,kd[]={
  SERVO_ROLL_D,SERVO_PITCH_D,NAV_ROLL_D,NAV_PITCH_D,THROTTLE_D,THROTTLE_FBW_D}; 		//PID gains

const float Integrator_max[]={
  SERVO_ROLL_INTEGRATOR_MAX,SERVO_PITCH_INTEGRATOR_MAX,NAV_ROLL_INTEGRATOR_MAX,NAV_PITCH_INTEGRATOR_MAX,THROTTLE_INTEGRATOR_MAX,THROTTLE_FBW_INTEGRATOR_MAX};

float	 Integrator[]={
  0,0,0,0,0,0};	//PID Integrators
float	 Last_error[]={
  0,0,0,0,0,0};	//PID last error for derivative
...
struct Location est_loc 			= {
  0,0,0};		// for estimation
...
void loop()
{
  ...
  if (DIYmillis()-fast_loopTimer > 19) {
    ...
    if (control_mode == FLY_BY_WIRE_A || control_mode == FLY_BY_WIRE_B) {
      ...
      // ----------------------------------------------------
      nav_roll = HEAD_MAX / 2;   		
      nav_pitch = abs(nav_roll * PITCH_COMP);
    ...
    case 4:
      ...
      //	This is the start of the slow (3 1/3 Hz) loop pieces
			switch (slow_loopCounter) {
       				...
       				case 1:
       					slow_loopCounter++;
       					// Would like to move this to the 50Hz section
       					// to do turn prediction
       					// --------------------------------------------
       					navigate();
       					break;
       				case 2:
